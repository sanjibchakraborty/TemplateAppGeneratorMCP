...
export const listModulesTool = {
  ...
  description:
    "Lists all available modules for iOS app generation. Use this to show the user which modules are available during the project creation wizard, before calling assemble_generation_prompt.",
  inputSchema: {
...
export function handleListModules(): string {
  ...
  const formatGroup = (
    ...
    const rows = modules.map(
      (m) =>
        `  • ${m.key.padEnd(26)} ${m.description}${m.dependencies ? ` [requires: ${m.dependencies.join(", ")}]` : ""}`
    );
  ...
  return [
    ...
    "",
    formatGroup(
      "Catalog Modules (add a visual catalog route to the app)",
      catalogModules
    ),
    "",
