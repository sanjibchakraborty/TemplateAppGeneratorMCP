import type { Architecture, AppSpec } from "../types.js";
import {
  validateModules,
  resolveGenerationPlan,
  resolveFinalAppStructName,
  ...
  inputSchema: {
    ...
    properties: {
      appName: {
        type: "string",
        description:
          "App name in PascalCase (e.g. MyApp, CardGame, FinanceApp). " +
      ...
      bundleId: {
        type: "string",
        description:
          "Reverse-domain bundle identifier (e.g. com.company.myapp). " +
      ...
      architecture: {
        ...
        description:
          "Architecture pattern for the project. " +
      ...
      modules: {
        ...
        items: { type: "string" },
        description:
          "List of module keys to include. Pass an empty array for a minimal app. " +
...
export async function handleAssembleGenerationPrompt(
  ...
  if (invalidModules.length > 0) {
    return errorResponse(
      "Unknown modules",
      `These module keys are not in the registry: ${invalidModules.join(", ")}\n` +
...
function validateAppName(name: string): string | null {
  if (!name || name.trim().length === 0) return "App name cannot be empty.";
  if (!/^[A-Z][a-zA-Z0-9]*$/.test(name)) {
    return (
      "App name must be PascalCase: start with an uppercase letter, " +
...
function validateBundleId(id: string): string | null {
  if (!id || id.trim().length === 0) return "Bundle ID cannot be empty.";
  if (!/^[a-z][a-z0-9]*(\.[a-z][a-z0-9]*)+$/.test(id)) {
    return (
      "Bundle ID must be reverse-domain format (e.g. com.company.myapp): " +
...
function buildPlanSummary(plan: ReturnType<typeof resolveGenerationPlan>): string {
  const moduleList =
...
function errorResponse(title: string, detail: string): string {
