...
export const MODULE_REGISTRY: ModuleDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Unified local UI package for CTAlertView, CTBottomSheetView, CTCarouselView, CTSnackBarView, and CTToastView",
    type: "catalog",
  ...
  },
  {
    ...
    description:
      "Configurable sidebar/drawer navigation container with gesture support",
    type: "catalog",
  ...
  },
  {
    ...
    displayName: "App Tab",
    description:
      "TabView dashboard root with per-tab NavigationStack and independent routers",
    type: "structural",
...
const CATALOG_MODULE_KEYS: ModuleKey[] = [
  "CTWalletKit",
  "Kingfisher",
  "CTPlayerKit",
  "TemplateUIPackage",
...
const APPTAB_CATALOG_TAB_TRIGGERS: ModuleKey[] = [
  "CTWalletKit",
  "Kingfisher",
  "CTPlayerKit",
];
...
export function validateModules(modules: string[]): {
  valid: ModuleKey[];
  invalid: string[];
} {
  ...
  for (const m of modules) {
    if (validKeys.has(m as ModuleKey)) {
      valid.push(m as ModuleKey);
    } else {
      invalid.push(m);
    }
  }
...
export function resolveFinalAppStructName(appName: string): string {
  return appName.endsWith("App") ? appName : `${appName}App`;
}

export function resolveGenerationPlan(spec: AppSpec): GenerationPlan {
  const { appName, bundleId, architecture, modules } = spec;

  const catalogModules = modules.filter((m) =>
    CATALOG_MODULE_KEYS.includes(m)
  );
  const serviceModules = modules.filter(
    (m) => !CATALOG_MODULE_KEYS.includes(m) && m !== "AppTab"
  );
  ...
  if (hasAppTab) {
    ...
    const hasCatalogTabTrigger = modules.some((m) =>
      APPTAB_CATALOG_TAB_TRIGGERS.includes(m)
    );
  ...
  const moduleContracts = [
    ...
    ...MODULE_REGISTRY.filter(
      (m) => catalogModules.includes(m.key)
    ).map((m) => m.contractFile),
    // Service modules next (in registry order)
    ...MODULE_REGISTRY.filter(
      (m) => serviceModules.includes(m.key)
    ).map((m) => m.contractFile),
    // AppTab always last
    ...(hasAppTab
...
export function formatModuleListForDisplay(): string {
  return MODULE_REGISTRY.map(
