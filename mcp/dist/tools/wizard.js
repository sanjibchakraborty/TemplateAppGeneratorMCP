import { validateModules, resolveGenerationPlan, MODULE_REGISTRY, } from "../lib/moduleRegistry.js";
...
export const wizardTool = {
    ...
    inputSchema: {
        ...
        properties: {
            appName: {
                type: "string",
                description: "PascalCase app name (e.g. MyApp, CardGame). " +
                    "Only set this after the user has explicitly typed their app name.",
            },
            bundleId: {
                type: "string",
                description: "Reverse-domain bundle ID (e.g. com.company.myapp). " +
                    "Only set after the user has provided or confirmed it.",
            },
            architecture: {
                type: "string",
                enum: ["MVVM+Builder+Router", "VIPER", "CleanArchitecture"],
                description: "Architecture pattern. Only set after the user has chosen one.",
            },
            modules: {
                type: "array",
                items: { type: "string" },
                description: "Module keys chosen by the user. Only set after the user has selected from the list. " +
                    "Pass an empty array [] explicitly if the user chose no modules.",
            },
            confirmed: {
                type: "boolean",
                description: "Set to true ONLY after the user has explicitly said 'yes', 'go', or a clear " +
...
// ─── Handler ──────────────────────────────────────────────────────────────────
export async function handleWizard(input) {
    ...
    if (modules === undefined) {
        const moduleOptions = MODULE_REGISTRY.map((m) => `    • "${m.key}" — ${m.description}`).join("\n");
        return [
    ...
    if (!confirmed) {
        ...
        const moduleList = validModules.length > 0 ? validModules.join(", ") : "None";
        return [
