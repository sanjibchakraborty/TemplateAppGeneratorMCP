...
export function getDocsPath(repoRoot) {
    return join(repoRoot, DOCS_REL);
}
export function isValidGeneratorRoot(root) {
 ...
 */
function getLegacyMonorepoGeneratorRoot() {
 ...
 */
async function fetchAndCheckoutRef(dir, ref) {
    ...
    try {
        await execGit("git", ["-C", dir, "fetch", "--depth", "100", "origin", branchSpec], { maxBuffer: 64 * 1024 * 1024 });
    ...
    await execGit("git", ["-C", dir, "fetch", "origin", branchSpec], { maxBuffer: 64 * 1024 * 1024 });
    // Bare `git checkout foo/bar` is parsed as ref + pathspec, not a branch name; use the remote tip.
    await execGit("git", ["-C", dir, "checkout", "--force", remoteTip]);
}
async function syncGitClone(url, ref, dir) {
...
function generatorResolutionError() {
    return new Error([
...
async function resolveGeneratorRootOnce() {
    const fromEnv = process.env[ENV_ROOT]?.trim();
    if (fromEnv) {
        const root = resolve(fromEnv);
        if (!isValidGeneratorRoot(root)) {
            throw new Error(`${ENV_ROOT} is set to "${root}" but ${getDocsPath(root)} was not found.`);
        }
        return root;
    }
    ...
    if (repoUrl) {
        ...
        if (!isValidGeneratorRoot(dir)) {
            throw new Error(`After cloning ${ENV_REPO_URL}, expected docs at ${getDocsPath(dir)} — check ref and repo layout.`);
        }
        return dir;
    }
 ...
 */
export function ensureGeneratorRoot() {
    if (!rootPromise) {
...
export function resetGeneratorRootCache() {
    rootPromise = null;
}
//# sourceMappingURL=generatorRoot.js.map